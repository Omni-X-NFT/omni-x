...
import {TransferManager} from "../core/TransferManager.sol";
...
contract OrderValidatorV2A {
    ...
    constructor(address _omniXExchange) {
        ...
        transferManager = omniXExchange.transferManager();
    ...
    function _deriveProtocolParameters() private {
        domainSeparator = omniXExchange.domainSeparator();
        creatorFeeManager = omniXExchange.creatorFeeManager();
        maxCreatorFeeBp = omniXExchange.maxCreatorFeeBp();
    }
    ...
    function _checkValidityNonces(
        ...
        if (omniXExchange.userSubsetNonce(makerSigner, subsetNonce)) {
            return USER_SUBSET_NONCE_CANCELLED;
        }

        // 2. Check order nonce
        bytes32 orderNonceStatus = omniXExchange.userOrderNonce(makerSigner, orderNonce);
        ...
        (uint256 globalBidNonce, uint256 globalAskNonce) = omniXExchange.userBidAskNonces(makerSigner);
    ...
    function _checkValidityCurrencyAndStrategy(
        ...
        if (!omniXExchange.isCurrencyAllowed(currency)) {
        ...
        (bool strategyIsActive, , , , , bool strategyIsMakerBid, address strategyImplementation) = omniXExchange
    ...
    function _checkValidityMakerBidERC20Assets(
        ...
        if (currency != address(0)) {
            ...
            if (IERC20(currency).allowance(user, address(omniXExchange)) < price) {
    ...
    function _checkValidityTransferManagerApprovals(address user) private view returns (uint256 validationCode) {
        if (!transferManager.hasUserApprovedOperator(user, address(omniXExchange))) {
            return NO_TRANSFER_MANAGER_APPROVAL_BY_USER_FOR_EXCHANGE;
        }

        if (!transferManager.isOperatorAllowed(address(omniXExchange))) {
    ...
    function _checkValidityMakerAskItemIdsAndAmountsAndPrice(
        ...
        if (makerAsk.strategyId == 0) {
            ...
            (, , , , bytes4 strategySelector, , address strategyImplementation) = omniXExchange.strategyInfo(
                makerAsk.strategyId
            );
