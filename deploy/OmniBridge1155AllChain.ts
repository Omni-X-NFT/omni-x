...
// Developers -  just need to run: $ npx hardhat run deploy/OmniBridge1155AllChain.ts --network <network>
...
async function main () {
  ...
  const contract = await deployContract(hre, 'OmniBridge1155', owner, [lzEndpoints[environments.testnet.indexOf(network)]])
  console.log('OmniBridge deployed to:', contract.address, 'on', network)

  shell.exec('npx hardhat verify --network ' + network + ' --contract ' + 'contracts/OmniBridge1155.sol:OmniBridge1155 ' + contract.address + ' ' + `${lzEndpoints[environments.testnet.indexOf(network)]}`).stdout.replace(/(\r\n|\n|\r|\s)/gm, '')
}
