...
import GREG_ARGS from '../constants/DadBroArgs.json'
  ...
  await Promise.all(
    networks.map(async (network: string) => {
      ...
      if (address) {
        ...
        const checkWireUpCommand = `npx hardhat verify --network ${network} ${address} "${aonftArgs.name}" "${aonftArgs.symbol}" ${endpointAddr} "${aonftArgs.baseTokenURI}" "${aonftArgs.hiddenURI}" ${aonftArgs.tax} ${aonftArgs.taxRecipient}`
