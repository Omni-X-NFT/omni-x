  ...
  await Promise.all(
    networks.map(async (network: string) => {
      ...
      if (address) {
        ...
        const checkWireUpCommand = `npx hardhat verify --contract contracts/token/onft721A/extension/collections/OmniAxAdventures.sol:OmniAxAdventures --network ${network} ${address} "${aonftArgs.name}" "${aonftArgs.symbol}" ${endpointAddr} ${aonftArgs.startId} ${aonftArgs.endId} ${aonftArgs.maxGlobalId} "${aonftArgs.baseURI}" "${aonftArgs.hiddenURI}" ${aonftArgs.tax} ${aonftArgs.price} ${aonftArgs.taxRecipient}`
