    ...
    await Promise.all(
      networks.map(async (network: string) => {
        networks.map(async (target: string) => {
          if (network !== target && network !== 'goerli' && network !== 'bsc-testnet' && network !== 'bsc-testnet' && network !== 'arbitrum-goerli' && network !== 'optimism-goerli' && network !== 'fantom-testnet' && network !== 'moonbeam_testnet' && network !== 'mumbai') {
